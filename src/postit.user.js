...
// @downloadURL  http://partydico.chez.com/postit.user.js
// @author       mens sana
...
// @resource     postitBackground http://imgur.com/Yutj3xB.png
...
var s = 'var postImg = { btnPost : "' + GM_getResourceURL("btnPost") + '",'
                      + 'boutCroix : "' + GM_getResourceURL("boutCroix") + '",'
                      + 'btnAutoNorm : "' + GM_getResourceURL("btnAutoNorm") + '",'
                      + 'btnAutoSelec : "' + GM_getResourceURL("btnAutoSelec") + '",'
                      + 'btnSort : "' + GM_getResourceURL("btnSort") + '",'
                      + 'btnClear : "' + GM_getResourceURL("btnClear") + '",'
                      + 'postitBackground : "' + GM_getResourceURL("postitBackground") + '"};';
...
var post = function()
{
    ...
    this.init = function()
    {
        ...
        document.body.addEventListener("dragover", drag_over, !1);
        document.body.addEventListener("drop", drop, !1);        

        this.visible = false;
        this.activAuto = false;
        this.barreLateraleVisible = true;
        this.isDragging = false;
        this.color = { norm: "#323741", valid: "#437331", util: "#6B6B6B", selec: "#242457" };  // selec  403C91  302E6E
	...
	{
		if(!(channel.data === undefined) && (channel.data.users.length > 0))
		{
    ...
    this.setListeners = function()
    {
        ...
        channel.socket.on("winWord", function(e)
        {
            ...
            var mot = removeDiacritics(t.lastWord.trim().toLowerCase());            
            var n = -1;
    ...
    this.popNewBox = function()
    {
        ...
        this.postBox.style.height = "291px",
        this.postBox.style.overflow = "hidden",
        this.postBox.style.background = "rgb(20, 20, 20)";
        this.postBox.addEventListener("dragstart", drag_start, !1);        
        ...
        this.createNewButton("btnAuto", postImg.btnAutoNorm, 0, 270, 66, 21, this.btnAutoClic, this.postBox);
        
        this.createNewButton("btnSort", postImg.btnSort, 66, 270, 67, 21, this.btnSortClic, this.postBox);

        this.createNewButton("btnClear", postImg.btnClear, 133, 270, 67, 21, this.btnClearClic, this.postBox);
        ...
        tableDiv.style.top = "35px",
        tableDiv.style.width = "174px",
        tableDiv.style.height = "197px",
    ...
    this.addMots = function(str)
    {
        ...
        for (var i = 0; i < ln; i++)
            ...
            {
                var mot = removeDiacritics(tab[i].trim().toLowerCase());
                if (mot)
                {
